        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_r and ctrl_held:
                    shape = 'RightTriangle'
                elif event.key == pygame.K_h and ctrl_held:
                    shape = 'rhombus'
...
def drawLineBetween(screen, index, start, end, width, color_mode,shape):
    ...
    for i in range(iterations):
        ...
        if shape == 'rectangle':
            pygame.draw.rect(screen,color,(x-width,y-width,width*2,width*3))
        elif shape == 'circle':
            ...
            pygame.draw.polygon(screen,color,[(x-width,y+width*(3**0.5)/3),(x+width,y+width*(3**0.5)/3),(x,y-width/(3**0.5)*2)])
        elif shape == 'rhombus':
            pygame.draw.polygon(screen,color,[(x-width,y),(x,y-width*2),(x+width,y),(x,y+width*2)])
main()